--[[
    ...
    Revision: 313 2010-08-27T19:47:03Z
...
function ArmoryFrame_Update(profile, refresh)
    ...
    if ( Armory:IsInspectCharacter() ) then
        PanelTemplates_SetTab(ArmoryFrame, 1);
        if ( not ArmoryPaperDollFrame:IsVisible() ) then
            ArmoryFrame_ToggleArmory("ArmoryPaperDollFrame");
        end
    end
    ...
    for _, realm in ipairs(realms) do
        ...
        for _, character in ipairs(Armory:CharacterList(realm)) do
            if ( not collapsed[realm] ) then
                ...
                if ( Armory:GetConfigShowNewbieTips() and Armory:UnitLevel(unit) and Armory:UnitRace(unit) and Armory:UnitClass(unit) ) then
                    characterList:SetCellScript(index, myColumn, "OnEnter", 
                        function(self, tooltipInfo)
                            self.oldnewbie = SHOW_NEWBIE_TIPS;
                            SHOW_NEWBIE_TIPS = "1";
                            GameTooltip_AddNewbieTip(self, tooltipInfo[1], 1.0, 1.0, 1.0, tooltipInfo[2], 1);
                        end,
                        {Armory:UnitPVPName(unit), format(PLAYER_LEVEL, Armory:UnitLevel(unit), Armory:UnitRace(unit), Armory:UnitClass(unit))}
                    ); 
...
function ArmoryFrame_UpdateLineTabs()
    ...
    if ( not Armory:IsInspectCharacter() ) then
...
function ArmoryFrameLeft_OnEnter(self)
    ...
    if ( profile ) then
        ...
        GameTooltip:SetScale(0.75 * Armory:GetConfigFrameScale());
        GameTooltip:Show();
...
function ArmoryComparisonFrame_OnUpdate(self, elapsed)
    ...
    if ( IsAltKeyDown() and tooltip ) then
        ...
        if ( not link or MerchantFrame:IsVisible() ) then
...
function ArmoryShowCompareItem(tooltip, link)
    ...
    if ( (link or "") == "" or not IsEquippableItem(link) ) then
        return;
    end
...
function Armory_oGlow_SetItemLink(self, button, link)
    if ( oGlow.CallFilters ) then
        oGlow:CallFilters("bags", button, link);
    elseif ( link ) then
...
function Armory_MIT_OnLoad()
    MIT:AddHook("Armory", "OnCreate", function(tooltip) Armory:RegisterTooltipHooks(tooltip); end);
end
...
function Armory_LW_OnLoad()
    LinkWrangler.RegisterCallback("Armory", function(tooltip) Armory:RegisterTooltipHooks(tooltip); end, "allocate"); 
end
...
function Armory_GS_OnLoad()
    ...
    ArmoryFrame_Update = function(...)
        ...
        if ( not success ) then
            ...
            score = 0;
        elseif ( IsAddOnLoaded("GearScore") ) then
            score = GS_Data[Armory.characterRealm].Players[Armory:UnitName("player")].GearScore;
        end
