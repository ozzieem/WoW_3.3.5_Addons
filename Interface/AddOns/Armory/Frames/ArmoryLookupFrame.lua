--[[
    ...
    Revision: 307 2010-07-21T14:43:50Z
...
local apiTable = {
    {id="01", label="Armor", func=Armory.UnitArmor},
    {id="02", label="ArmorPenetration", func=Armory.GetArmorPenetration},
    {id="03", label="AttackBothHands", func=Armory.UnitAttackBothHands},
    {id="04", label="AttackPower", func=Armory.UnitAttackPower},
    {id="05", label="AttackSpeed", func=Armory.UnitAttackSpeed},
    {id="06", label="BlockChance", func=Armory.GetBlockChance},
    {id="07", label="Class", func=Armory.UnitClass},
    {ids={
        {id="08", label="CombatRating", func=function(self, unit, index) return self:GetCombatRating(index) end},
        {id="09", label="CombatRatingBonus", func=function(self, unit, index) return self:GetCombatRatingBonus(index) end},
        {id="10", label="MaxCombatRatingBonus", func=function(self, unit, index) return self:GetMaxCombatRatingBonus(index) end},
     }, first=1, last=25 
    },
    {id="11", label="CritChance", func=Armory.GetCritChance},
    {id="12", label="CritChanceFromAgility",func=Armory.GetCritChanceFromAgility}, 
    {id="13", label="Damage", func=Armory.UnitDamage}, 
    {id="14", label="Defense", func=Armory.UnitDefense},
    {id="15", label="DodgeChance", func=Armory.GetDodgeChance},
    {id="16", label="Expertise", func=Armory.GetExpertise},
    {id="17", label="ExpertisePercent", func=Armory.GetExpertisePercent},
    {id="18", label="Guild", func=Armory.GetGuildInfo},
    {id="19", label="HasMana", func=Armory.UnitHasMana},
    {id="20", label="HasRelicSlot", func=Armory.UnitHasRelicSlot},
    {id="21", label="HasWandEquipped", func=Armory.HasWandEquipped},
    {id="22", label="Health", func=Armory.UnitHealth},
    {id="23", label="HealthMax", func=Armory.UnitHealthMax},
    {id="24", label="HealthRegenRateFromSpirit", func=Armory.GetUnitHealthRegenRateFromSpirit},
    {id="25", label="IsResting", func=Armory.IsResting},
    {id="26", label="Level", func=Armory.UnitLevel},
    {id="27", label="Mana", func=Armory.UnitMana},
    {id="28", label="ManaMax", func=Armory.UnitManaMax},
    {id="29", label="ManaRegen", func=Armory.GetManaRegen},
    {id="30", label="ManaRegenRateFromSpirit", func=Armory.GetUnitManaRegenRateFromSpirit}, 
    {id="31", label="MaxHealthModifier", func=Armory.GetUnitMaxHealthModifier},
    {id="32", label="ParryChance", func=Armory.GetParryChance},
    {id="33", label="PowerType", func=Armory.UnitPowerType},
    {id="34", label="PVPName", func=Armory.UnitPVPName},
    {id="35", label="Race", func=Armory.UnitRace},
    {id="36", label="RangedAttack", func=Armory.UnitRangedAttack},
    {id="37", label="RangedAttackPower", func=Armory.UnitRangedAttackPower},
    {id="38", label="RangedCritChance", func=Armory.GetRangedCritChance},
    {id="39", label="RangedDamage", func=Armory.UnitRangedDamage},
    {id="40", label="Resistance", func=Armory.UnitResistance, first=2, last=6},
    {id="41", label="Sex", func=Armory.UnitSex},
    {id="42", label="ShieldBlock", func=Armory.GetShieldBlock},
    {ids={
        {id="43", label="SpellBonusDamage", func=function(self, unit, index) return self:GetSpellBonusDamage(index) end},
        {id="44", label="SpellCritChance", func=function(self, unit, index) return self:GetSpellCritChance(index) end},
     }, first=2, last=MAX_SPELL_SCHOOLS
    },
    {id="45", label="SpellBonusHealing", func=Armory.GetSpellBonusHealing},
    {id="46", label="SpellCritChanceFromIntellect", func=Armory.GetSpellCritChanceFromIntellect},
    {id="47", label="SpellPenetration", func=Armory.GetSpellPenetration},
    {id="48", label="Stat", func=Armory.UnitStat, first=1, last=5},
    {id="49", label="SubZone", func=Armory.GetSubZoneText},
    {id="50", label="XP", func=Armory.UnitXP},
    {id="51", label="XPExhaustion", func=Armory.GetXPExhaustion},
    {id="52", label="XPMax", func=Armory.UnitXPMax},
    {id="53", label="Zone", func=Armory.GetZoneText},
    
    {id="54", label="Skills1", func=function(self) local skills=self:GetPrimaryTradeSkills(); if skills[1] then return unpack(skills[1]) end end},
    {id="55", label="Skills2", func=function(self) local skills=self:GetPrimaryTradeSkills(); if skills[2] then return unpack(skills[2]) end end},
    
    {id="56", label="Buff", func=function(self, unit, index) return self:GetBuff(unit, index, "HELPFUL") end, first=1, last=ARMORY_BUFF_MAX_DISPLAY},
    {id="57", label="Debuff", func=function(self, unit, index) return self:GetBuff(unit, index, "HARMFUL") end, first=1, last=ARMORY_DEBUFF_MAX_DISPLAY},
};
...
function ArmoryLookupFrame_OnEvent(self, event, ...)
    ...
    if ( event == "PLAYER_TARGET_CHANGED" or event == "PLAYER_GUILD_UPDATE"
          or event == "RAID_ROSTER_UPDATE" or event == "PARTY_MEMBERS_CHANGED" 
...
function ArmoryLookupChannelDropDown_Initialize()
    ...
    local channels = {
        ...
        RAID = function() return GetRealNumRaidMembers() > 0 end,
        PARTY = function() return GetRealNumPartyMembers() > 0 end
    };
...
function ArmoryLookupFrame_UpdateTarget()
    ...
    elseif ( channel == "RAID" ) then
        ArmoryLookupFrameTargetText:SetFormattedText(NUM_RAID_MEMBERS, GetRealNumRaidMembers());
        ...
        if ( search == "*" ) then
            ...
            for i = 1, numEntries do
                ...
                if ( skillType ~= "header" and ArmoryLookupFrame_IsMatch(skillName, search, exact) ) then
                    id = dbEntry:GetValue(container, i, "Data");
                    link = dbEntry:GetValue(id, "RecipeLink");
                    if ( link ) then
                        ref = link:match("|H"..refType..":(%d+)|h");
                        if ( ref ) then
                            if ( arg2 ) then
                                itemId = Armory:GetItemId(dbEntry:GetValue(id, "ItemLink"));
                                texture = dbEntry:GetValue(id, "Icon");
                                if ( texture ) then
                                    texture = table.remove({strsplit("\\", texture)});
                                end
                                numMade = dbEntry:GetValue(id, "NumMade");
                                if ( numMade == 1 ) then
                                    numMade = nil;
                                end
...
function ArmoryLookupFrame_InspectCharacter(exact, search, arg1)
    ...
    local inspect = true;
    local pet = false;
    local talentGroup = Armory:GetActiveTalentGroup(inspect, pet);
    ...
    if ( Armory:GetConfigShareCharacter() and strlower(Armory.character) == strlower(search) ) then
        ...
        local name, texture, pointsSpent, background, tier, column, rank, maxRank, isExceptional, meetsPrereq;
        for i = 1, Armory:GetNumTalentTabs(inspect, pet) do
            name, texture, pointsSpent, background = Armory:GetTalentTabInfo(i, inspect, pet, talentGroup);
            if ( texture ) then
                texture = table.remove({strsplit("\\", texture)});
            end
            tabs[i] = Armory:Join("|", name, texture, pointsSpent, background);
            for j = 1, Armory:GetNumTalents(i, inspect, pet) do
                _, texture, tier, column, rank, maxRank, isExceptional, meetsPrereq = Armory:GetTalentInfo(i, j, inspect, pet, talentGroup);
                link = Armory:GetTalentLink(i, j, inspect, pet, talentGroup);
                if ( link ) then
                    ref = link:match("|Htalent:([-%d:]+)|h");
                    if ( ref ) then
                        table.insert(info, Armory:Join("|", i, j, ref));
                        table.insert(info, Armory:Join("|", table.remove({strsplit("\\", texture)}), tier, column, rank, maxRank, isExceptional, meetsPrereq));
                        table.insert(info, Armory:Join("|", Armory:GetTalentPrereqs(i, j, inspect, pet, talentGroup)));
                    end
                end 
            end
        end
        ...
        if ( #info > 0 ) then
            table.insert(info, 1, table.concat(tabs, ";"));
            table.insert(result, table.concat(info, ARMORY_LOOKUP_CONTENT_SEPARATOR));
        end
        ...
        for _, api in ipairs(apiTable) do
            if ( api.ids ) then
                for i = api.first, api.last do
                    for _, api in ipairs(api.ids) do
                        if ( api.func(Armory, "player", i) ) then
                            table.insert(result, Armory:Join(ARMORY_LOOKUP_CONTENT_SEPARATOR, api.id..i, api.func(Armory, "player", i)));
                        end
                    end
                end
            elseif ( api.first and api.last ) then
                for i = api.first, api.last do
                    if ( api.func(Armory, "player", i) ) then
                        table.insert(result, Armory:Join(ARMORY_LOOKUP_CONTENT_SEPARATOR, api.id..i, api.func(Armory, "player", i)));
                    end
                end
            elseif ( api.func(Armory, "player", i) ) then
                table.insert(result, Armory:Join(ARMORY_LOOKUP_CONTENT_SEPARATOR, api.id, api.func(Armory, "player")));
            end
        end
    end
...
function ArmoryLookupFrame_ProcessResponse(id)
    ...
    for sender, reply in pairs(module.replies) do
        ...
        for _, set in ipairs(sets) do
            ...
            if ( id == ARMORY_LOOKUP_TYPE.LOOKUP_DOWNLOAD and tonumber(reply.version) >= 3 ) then
            ...
            elseif ( id == ARMORY_LOOKUP_TYPE.LOOKUP_CHARACTER and reply.version == "2" ) then
                -- owner\n(1)\n(2)\n(3)
                owner = fields[1];
                
                Armory:ResetInpectValues();

                local apiField = 4;

                -- (1) (19 items) 18821:0:0:0:0:0:0:0\f31333:0:0:0:0:0:0:1443794188
                values = ArmoryLookupFrame_ParseCharacterEquipment(owner, fields[2]);
                table.insert(data, {name=ARMORY_EQUIPMENT, values=values});

                -- (2) (talents) tab1;tab2;tab3\fref1\finfo1\fprereq1\fref2\finfo2\fprereq2
                ...
                -- (3) (values) key\f:N0\f\fS:String (numeric, nil, string)
                for i = apiField, #fields do
                    ArmoryLookupFrame_ParseAPI(owner, Armory:Split(ARMORY_LOOKUP_CONTENT_SEPARATOR, fields[i]));
                end
                
                table.insert(data, {name=ARMORY_LOOKUP_DETAIL, func=ArmoryLookupFrame_ToggleArmory, owner=owner});
                update = true;
...
function ArmoryLookupFrame_ParseCharacterEquipment(owner, fields)
    ...
    for i = 1, #values do
        name, link = Armory:GetInfoFromId("item", values[i]);
        if ( name ) then
            if ( link ) then
                -- Get it in the cache
                Armory:PrepareTooltip();
                Armory.tooltip:SetHyperlink(link);
            ...
            table.insert(result, {id=values[i], name=name.." "..GRAY_FONT_COLOR_CODE..slots[i], link=link, icon=texture});
            
            Armory:SetInspectValue(owner, "InventoryItem"..i, 1);
            Armory:SetInspectValue(owner, "InventoryItemLink"..i, link);
            Armory:SetInspectValue(owner, "InventoryItemTexture"..i, texture);
        end
    end
...
function ArmoryLookupFrame_ParseCharacterTalents(owner, fields)
    local values = Armory:StringSplit(ARMORY_LOOKUP_CONTENT_SEPARATOR, fields);
    local tabInfo = values[1];
    local result = {};
    local name;

    local tabs = Armory:StringSplit(";", tabInfo);
    table.wipe(build);

    if ( #tabs > 1 ) then
        local texture, pointsSpent, background;
        for i = 1, #tabs do
            name, texture, pointsSpent, background = Armory:Split("|", tabs[i]);
            if ( name ) then
                if ( texture ) then
                    texture = "Interface\\Icons\\"..texture;
                end
                table.insert(build, name..":"..pointsSpent);
                
                Armory:SetInspectTalentValue(owner, i, "Info", name, texture, pointsSpent, background);
            end
        end
        
        local ref, tab, id, link;
        local tier, column, rank, maxRank, isExceptional, meetsPrereq;
        table.wipe(tabs);
        for i = 2, #values, 3 do
            tab, id, ref = Armory:Split("|", values[i]);
            name, link = Armory:GetInfoFromId("talent", ref);
            if ( name ) then
                if ( tabs["NumTalents"..tab] ) then
                    tabs["NumTalents"..tab] = max(tabs["NumTalents"..tab], id);
                else
                    tabs["NumTalents"..tab] = id;
                end
                texture, tier, column, rank, maxRank, isExceptional, meetsPrereq = Armory:Split("|", values[i+1]);
                if ( texture ) then
                    texture = "Interface\\Icons\\"..texture;
                end
                if ( rank and rank > 0 ) then
                    table.insert(result, {id=ref, name=string.format("%s (%s/%s)", name, rank, maxRank), link=link, icon=texture});
                end

                Armory:SetInspectTalentValue(owner, tab, "NumTalents", tabs["NumTalents"..tab]);
                Armory:SetInspectTalentValue(owner, tab, "Info"..id, name, texture, tier, column, rank, maxRank, isExceptional, meetsPrereq);
                Armory:SetInspectTalentValue(owner, tab, "Link"..id, link);
                Armory:SetInspectTalentValue(owner, tab, "Prereqs"..id, Armory:Split("|", values[i+2]));
            end
        end

        name = table.concat(build, " ");
        table.wipe(build);
    end
...
function ArmoryLookupFrame_ParseAPI(owner, ref, ...)
    local id, index = ref:match("(..)(.*)");

    for _, api in ipairs(apiTable) do
        if ( api.ids ) then
            for _, api in ipairs(api.ids) do
                if ( api.id == id ) then
                    Armory:SetInspectValue(owner, api.label..index, ...);
                    return;
                end
            end
        elseif ( api.id == id ) then
            Armory:SetInspectValue(owner, api.label..index, ...);
            return;
        end
    end
...
function ArmoryLookupFrame_ToggleArmory(item)
    Armory:LoadInspectProfile(item.owner);
    if ( not ArmoryFrame:IsShown() ) then
        ShowUIPanel(ArmoryFrame);
    end
    ArmoryCloseChildWindows();
    
    Armory:Profiles(true);
    ArmoryFrame_Update(Armory:CurrentProfile(), true);
end
