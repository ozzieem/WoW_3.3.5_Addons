...
local function hookFrame(Gframe)
	...
	if Gframe == ChatEdit_GetActiveWindow() then
...
function AceTab:RegisterTabCompletion(descriptor, regex, wlfunc, usage, editframes)
	...
	if not editframes then
		...
		for i = 1, NUM_CHAT_WINDOWS do
			editframes[i] = _G["ChatFrame"..i.."EditBox"]
		end
	...
	for _, frame in pairs(editframes) do
		...
		if type(Gframe) ~= "table" or type(Gframe[0]) ~= "userdata" or type(Gframe.IsObjectType) ~= "function" then -- 3.3 hack (IsFrameType => IsObjecType)
			self:error("Cannot register frame %q; it does not exist", frame)
			frame = nil
		end
		
		if frame then
			if Gframe:GetObjectType() ~= "EditBox" then
