...
function Recount:TimeTick()
	...
	if Recount.TickTimeData then
	   local TimeCheck=Time-FilterSize-syncTime
	   local gotdeleted
	   ...
	   for name,v in pairs(Recount.db2.combatants) do
		 ...
		 for k, v2 in pairs(v.TimeWindows) do
		    ...
		    if NewEntry~=0 then
			  ...
			  TimeData[1][#TimeData[1]+1]=Time-syncTime-FilterMiddle
...
function Recount:OnEnable(first)
	...
	Recount:ScheduleRepeatingTimer("TimeTick", syncTime)
	Recount:Print("Syncing every " .. syncTime .. " seconds")
